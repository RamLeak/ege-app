...
| C2 | Мини-стейдж: спойлер ответов в ErrorCard + опечатка «непререшённых» → «неперерешённых» + диагностика смены варианта радара | ⏸ Waiting |
| D | Уведомления — пробники / streak / напоминания через WorkManager | ⏸ Waiting |

### Known issues для P3-C2 (отложено осознанно после Stage C)

- **Спойлер ответов в `ErrorCard`.** Сейчас правильный ответ показывается рядом с ошибочным сразу при открытии Журнала → Ошибки. Пользователь видит верный ответ ещё до того, как нажал «Перерешать» — это снижает ценность повторного решения. Решение: спрятать `correctAnswer` под «Показать ответ» (тап раскрывает) или показывать его только в режиме `fromErrors=true` после неверного ответа.
- **Опечатка «непререшённых».** В `JournalScreen.kt` и `ErrorsListScreen.kt` (subtitle и фильтр) используется форма «непререшённых» — правильно «неперерешённых» (с двумя «не»: «не» + «пере-» + «решённых»). Заменить везде через grep.
- **Смена варианта радара не отображалась после Настройки → Радар на главном.** Возможные причины: (1) у пользователя не было RED-подвидов, поэтому DONUT/HEATMAP скрылись с `EmptyHint`; (2) `HomeViewModel.state` подписан на `AppSettingsStore.settingsFlow.collectAsState`, но `LaunchedEffect(Unit) { refresh() }` срабатывает только при первом show — при смене радара UI должен перекомпозиться сам через collectAsState без refresh(), но возможно требуется явный recompose. Диагностика: добавить лог `Log.d("Radar", "style=${st.settings.radarStyle}")` в HomeScreen для проверки.
