...
27. **BackupSnapshot версионирование + backward-compat** — при добавлении новых Store'ов в бэкап **бампим minor-версию** (1.0 → 1.1), но старые бэкапы должны парситься. Реализация: `BackupSnapshot.CURRENT_VERSION = "1.1"`, `SUPPORTED_VERSIONS = setOf("1.0", "1.1")`, проверка через `BackupSnapshot.isSupported(version)`. Новые поля (в P3-B — `userStats: UserStatsSnapshot = UserStatsSnapshot()` и `streak: StreakState = StreakState()`) обязательно с **default-значениями**. `Json { ignoreUnknownKeys = true }` уже выставлен в BackupRepository — позволяет читать backup'ы с лишними полями (forward-compat для старых версий приложения, в которых поле было удалено). При удалении Store-поля — НЕ удалять из BackupSnapshot, оставлять для совместимости (помечать `@Deprecated`). При **изменении формата** существующего поля (например, переход с `String` дат на `Long` epoch) — нужен migration-code в `applyBackup()` под старую версию + бамп major-версии.
...
| C | Полноценный журнал — ошибки, статистика, user_attempts, экспорт CSV | ⏸ Waiting |
