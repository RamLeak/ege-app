"""Обходчик русского языка (sdamgia.ru).

Структурно идентичен math_profile.py — те же селекторы, тот же pipeline.
Отличия:
- HOST = rus-ege.sdamgia.ru
- JSONL = parser/russian.jsonl
- STATE = parser/state_russian.json
- subject_slug = 'rus'
- В русском у задач нет KES-кода и subtype_path (поля None) — это известное
  ограничение sdamgia (см. selectors.yaml).
- У задач №1-3 на sdamgia нет исходного текста, к которому они отсылают;
  парсим как есть, в Android-UI помечаем визуально.

Supplementary (всё после разделителя `<h3>Дополнительные задания для подготовки</h3>`
в каталоге) НЕ парсится по решению пользователя — устаревший формат ЕГЭ.

Запуск:
    python parser/scrapers/russian.py            # обычный прогон
    python parser/scrapers/russian.py --limit 50 # прогон первых 50 задач
    python parser/scrapers/russian.py --only-cat 365  # один подвид
"""
...
HOST = "https://rus-ege.sdamgia.ru"
SUBJECT = "russian"
JSONL_PATH = PARSER_ROOT / "russian.jsonl"
STATE_PATH = PARSER_ROOT / "state_russian.json"
...
def _serialize_problem(
    ...
    return {
        ...
        "kes_code": raw.kes_code,         # обычно None для русского
        "kes_title": raw.kes_title,       # обычно None для русского
        "subtype_path": raw.subtype_path,  # обычно None для русского
...
def run(limit: Optional[int] = None, only_cat: Optional[int] = None,
    ...
    with fetch_mod.make_client() as client:
        # 1. Каталог. Cache key русского отличается от математики чтобы не
        # затереть math_prob_catalog.html в общем cache/raw/.
        cat_res = fetch_mod.fetch(client, f"{HOST}/prob_catalog", label="rus_prob_catalog")
        ...
        for type_block, sub in targets:
            ...
            while True:
                ...
                label = f"rus_test_cat{sub.category_id}_p{page}"
...
def main() -> int:
    ...
    ap.add_argument("--include-supplementary", action="store_true",
                    help="включить устаревшие Д A*/Д B* (по умолчанию — только №1..№27)")
    args = ap.parse_args()
